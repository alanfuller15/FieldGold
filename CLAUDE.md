...
| `docs/fieldgold-chrome.css` | **shared page chrome** — the bottom bar every tool page carries, holding the back control to `index.html`. Static markup + this stylesheet, never a script: a page that throws is exactly when you need to leave it, and `creek_manual.html` loads no JS at all. The bar is at the BOTTOM by necessity, not taste — see the controls rule below. It is the seam Phase 1's routing consolidation grows into |
...
| `.claude/verify.sh` | **the verification gate.** Runs all thirteen suites plainly and refuses to report green over a suite it could not run |
...
| `tests/` | **thirteen suites, 570 assertions** — re-measured 2026-07-29 by running all thirteen and summing each suite's own printed total. Written as name:count pairs because the bare list this row used to carry could not be mapped back onto suites, and a session trying to work out which one was `39` had to re-measure every suite to find out: `test_land_status.js` 30, `test_photo_land_context.js` 50 (node); `test_app_land_status.py` 30, `test_photo_app.py` 29, `test_map_sites.py` 31, `test_offline_map.py` 80, `test_seed_drift.py` 15, `test_shell_divergence.py` 25, `test_stage_maps.py` 109, `test_state_claims.py` 68, `test_reactive_refresh.py` 25, `test_sw_lifecycle.py` 15, `test_panel_reachability.py` 63 (playwright). It read 563, 550, 525 and 510 earlier on 2026-07-29, 489 on 2026-07-26 and 488 before that; `test_offline_map.py` gained 21 in the false-✓ fix, `test_panel_reachability.py` 15 in the safe-area fix, and `test_shell_divergence.py` is new. **Twelve of the thirteen** take `--mutate`, **83 mutants** (81 distinct names — `no-banner` and `stale-cache` each appear in two suites). `test_app_land_status.py` does not, and that gap is open. **The "all caught" run covered 63 of them.** Counted off the tree: 57 mutants at `35adc6f`, 66 at `72e8815`, 66 on 2026-07-26, **83 on 2026-07-29** (+3 false-✓, +3 safe-area, +6 shell-divergence, +3 warn-surfacing, +2 kind-boundary; all seventeen verified caught). Count them with `MUTATE ===?` across `tests/` **plus** `test_seed_drift.py`'s `MUTANTS` dict — it dispatches from a dict rather than an `elif` chain, and the `.js` suites use `===`, so a naive grep undercounts by 18 and reports 51. The 63 was never the tree's count — three mutants have never appeared in a verified-all-caught run, and which three is not recorded. The conclusion that the 63 were caught still stands; only the number was wrong. **The exit convention is split and this file got it wrong twice** — verified empirically 2026-07-26 by running those 63. Inverting (caught → 0, survived → 1): `test_shell_divergence.py`, `test_offline_map.py`, `test_sw_lifecycle.py`, `test_reactive_refresh.py`, `test_seed_drift.py`, `test_panel_reachability.py`. Exit 1 on any failure: `test_map_sites.py`, `test_photo_app.py`, `test_stage_maps.py`, `test_state_claims.py`, and both `.js` suites. **Do not read this table — run one known mutant per suite and observe the code.** Whatever a runner assumes, half the suites report the opposite, and the failure mode is a runner that prints green over a survived mutant. A mutation whose `replace()` matches nothing must abort with exit 2 rather than report a pass — but see below: exit 2 does not catch everything |
...
### The rule extends to controls, and the finger is the only authority

**A notice behind a button that cannot be pressed has been removed as surely as
one below an unreachable fold.** The rule above was written about text. The
device run found the identical failure one level up, on the control: `map.html`
auto-collapses its panel at phone width, and the toggle that reopens it measured
`[22,24,153,19]` on an iPhone 14 — y=22..41, wholly inside the 47px status bar.
Alan could not press it. Every land-status warning in that panel was therefore
unreadable, while the reachability suite reported **291 of 291 sample points
reachable, FAILS 0** — because the measurement opened the panel with
`classList.remove('collapsed')`, a route the user does not have.

**A green obtained by a route the user does not have is not a green.** Same
family as `el.scrollTop = n` succeeding on an `overflow-y:hidden` element, and
the trap is now recorded at both levels: on the content, and on the control that
reveals it.

**And no automated test in this repo can settle it.** The iOS status bar is
native chrome composited above a full-bleed webview; the page cannot see it, so
`elementFromPoint` returned the toggle as hit-testable on a control iOS was
eating the touches for — demonstrated twice, on two installs, one of them
virgin. Every hit test here is blind to native chrome and will pass over an
untappable control. **The authority for tappability is Alan's finger, and the
authority for what a person actually sees is a photograph of the screen.**

Two consequences for anything you build near a screen edge:

- **Put controls where the instruments can see.** Native chrome is at the *top*.
  A bottom control's only native neighbour is the home indicator, which is a
  padding problem, not a hit-testing one. That is why the Phase 1 navigation and
  problem bars go at the bottom — not a preference, it is where an automated
  assertion can establish reachability at all.
- **Assert geometry, not hit-testability, for anything in the top strip** —
  "is this control below the inset" is checkable; "can a finger reach it" is not.
...
scroller, so scrolling the panel can never reach its lower lines. **That
exclusion is correct and it covered the app's entire failure-reporting
channel** — all thirteen suites passed green against a build whose no-signal
warning no user could read. The gap is closed by `tests/test_offline_map.py`
section 8, which asserts a warn or err is on screen with NO interaction at
all, not by removing the exclusion.

**A land-status warning and a failure report are different kinds and are not
ranked against each other.** `tally()` logs `warn` whenever a bench sits on
encumbered ground — that is the app *working* and saying something
important. "basemap tiles unavailable" is the app saying what it *could not
do*. A single ordered list forces one to displace the other: first-line
headlined the REM tally while the screen was black and unexplained, and
severity-first promotes `geochem markers failed` over the no-signal line
that explains it. The problem row headlines the first **failure**; the
land-status line keeps its `warn` class, its place in the run log and its
place in the expanded list, and takes the headline when nothing has failed.
**Classify at the call site — `log(msg, cls, kind)` — never by matching on
message text.** A line's kind is a property of what produced it; a regex
over wording breaks silently the first time someone rephrases a warning.
**Failure is the default and land-status opts in**, so a warning added later
without a kind is over-reported rather than silently omitted.
