...
| `PRIOR-ART-PHASE1.md` | 711 | **the prior-art pass over `PHASE1-DESIGN.md`, run 2026-07-29 before any of it was built.** Six items in the protocol's report shape, both queries recorded verbatim for each. None returned "no prior art found". Two conflicts and four gaps are collected at the end, reported rather than resolved — the design was not amended by this pass |
| `.claude/skills/prior-art/SKILL.md` | 137 | **the prior-art protocol.** Four steps: rename the thing in field vocabulary, search that name, run a second search that is mechanically different in at least two named dimensions, report in a fixed shape. Exists so "I searched and found nothing" becomes a claim that can be wrong. Use before designing any mechanism this project has not built before |
